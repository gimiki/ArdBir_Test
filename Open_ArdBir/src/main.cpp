...
//! Setting 8 Stages
float p_C[] = { 75.00, 20.00, 0.25, 55.00, 25.00, 0.25, 50.00, 35.00, 0.25,
		60.00, 45.00, 0.25, 70.00, 50.00, 0.25, 76.00, 60.00, 0.25, 76.00,
		60.00, 0.25, 80.00, 75.00, 0.25 };
float p_F[] = { 167.00, 68.00, 0.25, 131.00, 77.00, 0.25, 122.00, 95.00, 0.25,
		140.00, 113.00, 0.25, 158.00, 122.00, 0.25, 168.75, 140.00, 0.25,
		176.00, 167.00, 0.25, 176.00, 167.00, 0.25 };

int p_PID[] = { 100, -100, 1, 400, -200, 1, 100, -100, 1, 7500, 1000, 250, 100,
		0, 1, 50, -50, 1 };
byte p_Unit[] = { 1, 0, 1, 1, 0, 1, 105, 90, 1, 221, 194, 1, 15, 5, 1, 5, 0, 1,
		1, 0, 1, 0, 0, 0, 1, 0, 1, 90, 0, 1 };

//! Specify the links and initial tuning parameters
PID myPID(&Input, &Output, &Setpoint, 95, 40, 0, DIRECT);

byte SP_Symbol[8] = { B11100, B10000, B11100, B00111, B11101, B00111, B00100,
		B00100 };  // [2] SP Symbol
byte PumpONOFF[8] = { B00000, B01110, B01010, B01110, B01000, B01000, B01000,
		B00000 };  // [3] Pump Symbol
byte RevPumpONOFF[8] = { B11111, B10001, B10101, B10001, B10111, B10111, B10111,
		B11111 };  // [4] Reverse PUMP Symbol
...
/*! \fn void Gradi()
 * \brief Degree symbol for LCD.
 * \var degC[] Degree C sybmol.
 * \var degF[] Degree F sybmol.
 */

void Gradi() {
	if (ScaleTemp == 0) {
		byte degC[8] = { B01000, B10100, B01000, B00111, B01000, B01000, B01000,
				B00111 };
		lcd.createChar(0, degC);  /// Celsius
	} else {
		byte degF[8] = { B01000, B10100, B01000, B00111, B00100, B00110, B00100,
				B00100 };
...
/*! \fn heat_on
 *
 * This function turns on the heater element.
 * \sa ledHeatON()
 */
void heat_on() {
...
/*! \fn heat_off(boolean mheat)
 *
 * 	This function turns off the heater element.
 * 	\sa ledHeatStatus(boolean mheat)
 */
...
/*! \fn pump_off(boolean mpump)
 *
 * 	This function turns off the pump and the led.
 * 	\sa ledPumpStatus(boolean mpump)
 */
...
/*! \fn allOFF()
 *
 * 	This function turns off the heater element and the pump.
 * 	\sa pump_off(boolean mpump)
 * 	\sa heat_off(boolean mheat)
 */
...
void PID_HEAT(boolean autoMode) {
	...
	if (autoMode) {
		...
		if ((Setpoint - Input) < DeltaPID * 1.2)
			myPID.Compute();  //was 6, getting close, start feeding the PID -mdw
		if ((Setpoint - Input) > DeltaPID)
			Output = 100; // was 5, ignore PID and go full speed -mdw  // set the output to full on
	}

	// PWM the output
	unsigned long now = millis();
	if (now - w_StartTime > WindowSize)
		w_StartTime += WindowSize; //time to shift the Relay Window
...
void pump_rest(byte stage) {
	...
	if (ScaleTemp == 0)
		r_set(TempPumpRest, 22);
	else
		r_set(TempPumpRest, 23);
	...
	if (Temp_Now >= TempPumpRest) {
		...
		if (stage == 8) {
			...
			if ((pumpTime < TimePumpCycle)) { // starts pumps and heat
				...
				if (pumpTime >= (TimePumpCycle + TimePumpRest)
						|| ((stageTemp - Temp_Now) > DeltaTemp))
					pumpTime = 0;
			}
		}
	}
...
void Timing(byte stage, boolean Test, byte Type) {
	...
	if ((millis() - start) > 1000) {  // timing routine
		...
		if (!(Test))
			second = 0;  // starts counting down when temp reached
		else {
			TimeLeft--;
			TimeSpent++;
		}
...
void Iodine_Test() {
	...
	if (ScaleTemp == 0)
		read_set(Setpoint, StageAddr - 5);
	else
		read_set(Setpoint, StageAddr - 3);
	...
	while (Test) {
		...
		if (btn_Press(Button_start, 50)
				|| IodineTime != 0 && TimeSpent >= IodineTime * 60) {
			Test = false;
			IodineTest = true;
			TimeSpent = 0;
	...
	while ((stageTime > 0) && (b_Enter)) {
		...
		if (pumpRest) {
			...
			if ((x - 1) == 8 && tempBoilReached && Temp_Now >= boilStageTemp) { //if temp reached during boil
				...
				if ((x - 1) == 8) {
					if (ScaleTemp == 0)
						Set(stageTemp, 110, EEPROM.read(17), 0.25, Timer,
								Verso);
					else
						Set(stageTemp, 230, EEPROM.read(18), 0.25, Timer,
								Verso);
					NoBoil();
		...
		if (Temp_Now >= stageTemp) {
			if ((x - 1) == 8 && tempBoilReached == false)
			...
			if (tempReached == false) {
				...
				if ((x - 1) == 8)
					hop_add();  // check for hop add once temp reached
			}
		}
...
void remove_malt() {
	...
	if (EEPROM.read(24) == 1 && SensorType == 0)
		wait_for_confirm(malt, 1, 2, 1);
	else
...
void manual_mode() {
	...
	while (manualLoop) {            // manual loop
		...
		if (Setpoint >= boilStageTemp && Input >= Setpoint) {
			...
			if (mpump)
				pump_rest(8); //Forced Boil Stage for Pump Control

		} else {
			if (ScaleTemp == 0)
				Set(mset_temp, 110, 20, 0.25, Timer, Verso);
			else
				Set(mset_temp, 230, 68, 0.25, Timer, Verso);

			if (mheat)
				PID_HEAT(true); //set heat in auto mode
			if (mpump)
				pump_rest(99);    //Forced normal Stage for jump Pump Control
		}
...
//  check_for_resume();
	...
	if (b_Enter) {     // mash steps
		...
		for (byte i = x; i < 8; i++) {
			...
			if (ScaleTemp == 0)
				read_set(stageTemp, StageAddr);
			else
				read_set(stageTemp, StageAddr + 2);
...
void set_PID() {
	...
	for (byte i = 0; i < 6; i++) {
		...
		if (i < 4 || i == 5)
			read_set(pidSet, setAddr);
		else
			r_set(boil_output, setAddr);
		pidLoop = true;
		...
		while (pidLoop) {
			...
			if (i == 4) {
				...
				Set(boil_output, p_PID[a], p_PID[a + 1], p_PID[a + 2], Timer,
						Verso);
			} else {
				PidSet(pidSet, i);

				if (i < 3)
					Set(pidSet, p_PID[a], p_PID[a + 1], p_PID[a + 2], Timer,
							Verso);
				else
					Set(pidSet, p_PID[a], p_PID[a + 1], p_PID[a + 2], Timer,
							Verso);
...
void set_Unit() {
	...
	for (byte i = 0; i < 10; i++) {
		a = i * 3;
		if ((i == 2 && ScaleTemp == 1) || (i == 3 && ScaleTemp == 0))
			unitLoop = false;
		else
			unitLoop = true;

		if (i != 7)
			r_set(unitSet, setAddr);
		else {
			if (ScaleTemp == 0)
				r_set(unitSet, 22);
			else
		...
		while (unitLoop) {
			...
			if (i != 7) {
				Set(unitSet, p_Unit[a], p_Unit[a + 1], p_Unit[a + 2], Timer,
						Verso);
			} else {
				if (ScaleTemp == 0) {
	...
	for (byte i = 0; i < nmbrHops; i++) {
		...
		while (hopLoop) {
			...
			if (i == 0)
				...
				if (i == 1)
					r_set(TimeUp, blhpAddr - 1);
				else
					TimeUp = EEPROM.read(blhpAddr - 1) - 1;
...
void set_Stages() {
	...
	for (byte i = 0; i < 8; i++) { // loops for the number of stages
		...
		if (ScaleTemp == 0) {
			...
			if (i > 0)
				read_set(temp_stageTemp, StageAddr - 5);
			...
			if (i > 0)
				read_set(temp_stageTemp, StageAddr - 3);
			temp_stageTemp = temp_stageTemp / 16.0;
			Max = p_F[a];
			Min = p_F[a + 1];
			Step = p_F[a + 2];
		...
		if (i > 0) {
			...
			//Salta lo Stage
				...
				if (MashInFLAG) {
					if (MashInTemp - DeltaTemp >= Min)
						Min = MashInTemp - DeltaTemp;
				} else {
					if (Min < temp_stageTemp)
						Min = temp_stageTemp;
				}
			}
		}
		while (TempTimeLoop) {  // loops for temp adjust
			...
			if (btn_Press(Button_start, 50)) {
				...
				if (ScaleTemp == 0)
					stagetempSet = p_C[a + 1];
				else
					stagetempSet = p_F[a + 1];
	...
	for (byte i = RicettaDwn + 89; i < RicettaUp + 89 + 1; i++) { //Trova la prima ricetta libera
		...
		while (ricettaLoop) {
			...
			for (pos = 0; pos < 10; pos++) {
				LCD_NomeRicetta(pos,
						EEPROM.read(620 + pos + ((numRicetta - 1) * 10)));
			}
...
void saveRecipe() {
	...
	if (numRicetta == 0)
		...
		while (pos < 10) {
			...
			if ((NomeRicetta[pos] > 32 && NomeRicetta[pos] < 48) && Verso == 1)
				NomeRicetta[pos] = 48;
			if ((NomeRicetta[pos] > 57 && NomeRicetta[pos] < 97) && Verso == 1)
				NomeRicetta[pos] = 97;

			if ((NomeRicetta[pos] < 97 && NomeRicetta[pos] > 57) && Verso == 2)
				NomeRicetta[pos] = 57;
			if (NomeRicetta[pos] < 48 && Verso == 2)
				NomeRicetta[pos] = 32;
...
void loop() {
	...
	default:
		...
		if (btn_Press(Button_dn, 500))
			mainMenu = 1;
		if (btn_Press(Button_start, 500))
			mainMenu = 2;
		if (btn_Press(Button_enter, 500))
			mainMenu = 3;
